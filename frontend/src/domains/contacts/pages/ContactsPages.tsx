...
import { Can } from "../../../app/shared/rbac";
...
function TemplateApprovalPage() {
  ...
  const load = async (cursorValue?: string) => {
    ...
    const [templateRows, summaryRow, runRows] = await Promise.all([
      api<unknown>(`/templates?${toQueryString({
    ...
    const normalized = normalizeQueryPage<WhatsAppTemplate>(templateRows);
    setTemplates(normalized.items);
    setCursor(cursorValue || "");
    setNextCursor(normalized.nextCursor || "");
    setTotalApprox(normalized.totalApprox);
    setSummary(summaryRow);
    setRuns(runRows);
    if (normalized.items.length && !selectedId) setSelectedId(normalized.items[0].id);
  };

  useEffect(() => {
    load("").catch((e) => setMsg(toUserErrorMessage(e)));
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filter]);
  ...
  return (
    <div className="template-page">
      ...
      <main className="template-main">
        ...
        <section className="template-workbench"><div className="template-list-card"><div className="template-toolbar"><input placeholder="Search templates..." value={search} onChange={(e) => setSearch(e.target.value)} /><button onClick={() => load("").catch((e) => safeLogClientError("ui_async_failed", e, { exposeToConsole: import.meta.env.DEV }))}>Search</button></div><div className="template-filters">{[["all","All"],["approved","Approved"],["pending","Pending"],["rejected","Rejected"],["draft","Draft/unknown"]].map(([id,label]) => <button key={id} className={filter === id ? "active" : ""} onClick={() => setFilter(id)}>{label}</button>)}</div><div className="template-list">{templates.map((t) => <article key={t.id} className={selected?.id === t.id ? "active" : ""} onClick={() => setSelectedId(t.id)}><div><strong>{t.name}</strong><span>{t.category || "uncategorized"} � {t.language}</span></div><em className={`tpl-status ${String(t.status).toLowerCase()}`}>{t.status}</em></article>)}</div><div className="campaign-actions"><span>{totalApprox != null ? `${totalApprox} approx` : ""}</span><button disabled={!nextCursor} onClick={() => load(nextCursor).catch((e) => safeLogClientError("ui_async_failed", e, { exposeToConsole: import.meta.env.DEV }))}>Next Page</button></div></div>
...
function WhatsAppSetupPage() {
  ...
  const readyToSend =
    String(status?.onboarding_state || "").toUpperCase() === "ONBOARDING_COMPLETE" &&
    Boolean(status?.cloud_api_registered) &&
    messageTestPassed &&
    deliveryWebhookHealthy;
  const steps = [
    { label: "Detected phone number", done: hasPhone, detail: hasPhone ? status?.display_phone_number || status?.phone_number_id || "Detected" : "Phone number not detected" },
    { label: "Register number", done: Boolean(status?.cloud_api_registered), detail: status?.cloud_api_registered ? "Cloud API registered" : "Registration required" },
    { label: "Verify display name", done: String(status?.display_name_status || "").toUpperCase() === "APPROVED", detail: status?.display_name_status || "Display name verification pending" },
    { label: "Subscribe webhooks", done: Boolean(status?.webhook_subscribed), detail: status?.webhook_subscribed ? "Subscribed" : "Subscription pending" },
    { label: "Send test message", done: messageTestPassed, detail: messageTestPassed ? "Synthetic test send succeeded" : "Run test send" },
    { label: "Receive delivery webhook", done: deliveryWebhookHealthy, detail: deliveryWebhookHealthy ? "Delivery webhook healthy" : "Waiting for delivery webhook" },
    { label: "Ready", done: readyToSend, detail: readyToSend ? "Onboarding complete" : "Complete remaining checks" },
  ];
...
function EmbeddedSignupNextPage() {
  ...
  const ready = String(status?.onboarding_state || "").toUpperCase() === "READY_TO_SEND";
            ...
            {templates.map((c) => (
              <div key={String(c.public_id)} className={`meta-conv-item ${selectedId === String(c.public_id) ? "active" : ""}`} onClick={() => setSelectedId(String(c.public_id))}>
                ...
                <div className="meta-conv-info">
                  ...
                  <div className="meta-conv-preview">{c.status} Ã‚Â· {(c.unread_count || 0)} unread</div>
                </div>
          ...
          <div className="content">
                      ...
                      {embeddedContacts.map((c) => (
                        <article className="embedded-contact-card" key={c.id}>
                          ...
                          <div className="embedded-contact-main">
                            ...
                            <div className="embedded-contact-meta">{c.status} Â· {c.lastMessageAt ? relTime(c.lastMessageAt) : "-"}</div>
                          </div>
                        ...
                        <div className="embedded-queue-list">
                          ...
                          {!queueLoading && campaignQueue.map((c) => (
                            <article key={c.campaign_id}>
                              ...
                              <span>{String(c.status || "draft")} Â· {(Number(c.total_recipients || c.eligible_recipients || 0)).toLocaleString()} recipients</span>
                            </article>
                          ))}
                        </div>
              ...
              <div className="conv-list">
                {templates.map((c) => (
                  <div key={String(c.public_id)} className={`conv-item ${selectedId === String(c.public_id) ? "active" : ""}`} onClick={() => setSelectedId(String(c.public_id))}>
                    <div className="conv-avatar" style={{ background: "#E7F9EE", color: "#1B7A42" }}>{initials(c.visitor_name)}<span className="resolved-tick">âœ“</span></div>
            ...
            <div className="chat-area">
              <div className="chat-header">
                <div className="conv-avatar chat-avatar" style={{ width: 36, height: 36, background: "#E7F9EE", color: "#1B7A42" }}>{initials(selectedConversation?.visitor_name || detail?.visitor_name)}<span className="resolved-tick">âœ“</span></div>
                <div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 500, color: "var(--color-text-secondary)" }}>{selectedConversation?.visitor_name || detail?.visitor_name || "Conversation"}</div><div style={{ fontSize: 10, color: "var(--color-text-tertiary)" }}>Resolved Â· {relTime(selectedConversation?.last_message_at)} Â· by Arjun J.</div></div>
                <span className="resolved-pill">âœ“ Resolved</span><button className="reopen-btn" onClick={() => reopenConversation().catch((e) => safeLogClientError("ui_async_failed", e, { exposeToConsole: import.meta.env.DEV }))}>Reopen</button>
              </div>
              <div className="resolved-summary-strip">âœ“ Resolved in <strong>{relTime(selectedConversation?.last_message_at)}</strong> Â· {messages.length} messages Â· CSAT: <strong>5/5 *</strong></div>
              <div className="chat-messages">
                ...
                <div className="resolved-card"><div className="resolved-card-icon">âœ“</div><div className="resolved-card-title">Conversation resolved</div><div className="resolved-card-copy">by Arjun J. Â· Duration: {relTime(selectedConversation?.last_message_at)}</div><div className="resolved-card-rating"><div>Customer rated this conversation</div><span className="star-row"><span className="star-on">*</span><span className="star-on">*</span><span className="star-on">*</span><span className="star-on">*</span><span className="star-on">*</span></span></div></div>
            ...
            <div className="right-panel">
              ...
              <div className="panel-section">
                ...
                <div className="stack" style={{ marginTop: 8 }}>
                  ...
                  <input placeholder="Amount (e.g. â‚¹4,800)" value={purchaseAmount} onChange={(e) => setPurchaseAmount(e.target.value)} />
...
function ContactsCrmPage() {
  ...
  const bulkAddTag = async () => {
    if (selection.mode !== "all_matching_query" || !selection.querySnapshotId) {
      setSaveMsg("Use Select all matching query before bulk tag operations.");
      return;
    }
    if (!bulkTagValue.trim()) return;
    setSaveMsg("Adding tag...");
    try {
      const response = await api<{ operation_id: string; affected_count?: number }>("/contacts/bulk-tag", "POST", {
        operation_id: `bulk-tag-${Date.now()}`,
        selection_mode: "snapshot",
        selection_snapshot_id: selection.querySnapshotId,
        action: "add",
        tag: bulkTagValue.trim(),
      });
      setSaveMsg(`Tag add operation queued (${response.operation_id}).${response.affected_count != null ? ` Affected: ${response.affected_count}` : ""}`);
      await load();
    } catch (e) {
      setSaveMsg(toUserErrorMessage(e));
    }
  ...
  const bulkRemoveTag = async () => {
    ...
    try {
      ...
      setSaveMsg(`Tag remove operation queued (${response.operation_id}).${response.affected_count != null ? ` Affected: ${response.affected_count}` : ""}`);
      await load();
    } catch (e) {
      setSaveMsg(toUserErrorMessage(e));
    }
  ...
  const bulkSuppress = async () => {
    if (selection.mode !== "all_matching_query" || !selection.querySnapshotId) {
      setSaveMsg("Use Select all matching query before suppressing contacts.");
      return;
    }
    setSaveMsg("Applying suppression...");
    try {
      const preview = await api<{ affected_count: number; confirmation_hash: string }>("/contacts/bulk-suppress/preview", "POST", {
        selection_mode: "snapshot",
        query_snapshot_id: selection.querySnapshotId,
      });
      const response = await api<{ status: string; suppressed_count?: number }>("/contacts/bulk-suppress/confirm", "POST", {
        selection_mode: "snapshot",
        query_snapshot_id: selection.querySnapshotId,
  ...
  const bulkExport = async () => {
    if (selection.mode !== "all_matching_query" || !selection.querySnapshotId) {
      setSaveMsg("Use Select all matching query before exporting contacts.");
      return;
    }
    ...
    try {
      ...
      if (status.status === "completed" && status.download_url) {
        const downloadHref = safeDownloadUrl(status.download_url);
  ...
  return (
    <Layout>
      ...
      <div className="campaign-actions">
        ...
        <Can permission="contacts.bulk_edit"><button onClick={() => selectAllMatchingQuery().catch((e) => safeLogClientError("ui_async_failed", e, { exposeToConsole: import.meta.env.DEV }))} disabled={!appliedQueryHash}>Select all matching query</button></Can>
        <button onClick={clearSelection}>Clear selection</button>
        <Can permission="contacts.bulk_tag"><button onClick={() => bulkAddTag().catch((e) => safeLogClientError("ui_async_failed", e, { exposeToConsole: import.meta.env.DEV }))}>Add tag</button></Can>
        <Can permission="contacts.bulk_tag"><button onClick={() => bulkRemoveTag().catch((e) => safeLogClientError("ui_async_failed", e, { exposeToConsole: import.meta.env.DEV }))}>Remove tag</button></Can>
        <Can permission="contacts.export"><button onClick={() => bulkExport().catch((e) => safeLogClientError("ui_async_failed", e, { exposeToConsole: import.meta.env.DEV }))}>Export</button></Can>
        <Can permission="contacts.bulk_suppress"><button onClick={() => bulkSuppress().catch((e) => safeLogClientError("ui_async_failed", e, { exposeToConsole: import.meta.env.DEV }))}>Suppress</button></Can>
  ...
  return (
    <div className="contacts-replica">
      ...
      <div className="shell">
        ...
        <main className="main"><div className="topbar"><div className="topbar-crumb">Command Center â€º <span>Contacts</span></div><div className="topbar-meta"><div className="meta-chip">{businessName}</div><div className="meta-chip">WABA: {wabaLabel}</div><div className="meta-chip">Last 7 days</div><button className="btn-logout" onClick={() => { clearAuthStorage(); nav('/login', { replace: true }); }}>Logout</button></div></div>
    ...
    <Layout>
      ...
      <section className="offline-campaigns">
        ...
        <ul>
          ...
          <li>Send to customers with bill value above â‚¹2,000</li>
          ...
          {!record ? <p>Select a customer.</p> : (
            <>
              <div className="customer-book-kpis">
                ...
                <div><span>Lifetime value</span><strong>{lifetimeValue ? `â‚¹${lifetimeValue}` : "-"}</strong></div>
