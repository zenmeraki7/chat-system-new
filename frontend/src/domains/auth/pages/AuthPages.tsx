...
function RequireAuth({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (devBypassEnabled) {
      ...
      if (!sessionStorage.getItem("auth_business_id")) sessionStorage.setItem("auth_business_id", "dev-business");
      setAllowed(true);
      setChecking(false);
      return;
    }
...
function LoginPage() {
  ...
  const finishLogin = async (loginEmail: string, loginPassword: string) => {
    ...
    const role = String(result.memberships?.[0]?.role || "agent").toLowerCase();
    sessionStorage.setItem("auth_role", role);
    sessionStorage.setItem("auth_permissions", JSON.stringify([]));
    try {
...
function WhatsAppSafetyScoreCard({ context }: { context: SafetyContext }) {
  const config: Record<SafetyContext, { score: number; revenue: string; fatigue: string; quality: string; support: string; recommendation: string }> = {
    campaign: { score: 84, revenue: "High", fatigue: "Low", quality: "Medium", support: "Manageable", recommendation: "Send in 3 batches" },
    template: { score: 81, revenue: "High", fatigue: "Low", quality: "Medium", support: "Manageable", recommendation: "Send in 3 batches" },
    automation: { score: 86, revenue: "High", fatigue: "Low", quality: "Low", support: "Manageable", recommendation: "Run in 3 staggered windows" },
    segment: { score: 84, revenue: "High", fatigue: "Low", quality: "Medium", support: "Manageable", recommendation: "Send in 3 batches" },
    phone_number: { score: 83, revenue: "High", fatigue: "Low", quality: "Medium", support: "Manageable", recommendation: "Warm up in 3 batches" },
  };
  const label: Record<SafetyContext, string> = {
    campaign: "Campaign",
    template: "Template",
  ...
  return (
    <section className="signature-safety-card">
      ...
      <div className="signature-safety-score">{c.score}<small>/100</small></div>
      <div className="signature-safety-grid">
        <div><span>Revenue Potential</span><strong>{c.revenue}</strong></div>
        <div><span>Customer Fatigue</span><strong>{c.fatigue}</strong></div>
        <div><span>Quality Risk</span><strong>{c.quality}</strong></div>
        <div><span>Support Load</span><strong>{c.support}</strong></div>
      </div>
      <p className="signature-safety-note"><strong>Recommended:</strong> {c.recommendation}</p>
    </section>
  );
}
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          <Can permission="templates.sync"><Link to="/templates">Templates</Link></Can>
          <Link to="/automations">Automations</Link>
          <Link to="/commerce">Commerce</Link>
          <Link to="/analytics">Analytics</Link>
          <Can permission="billing.read"><Link to="/billing">Billing</Link></Can>
          <Link to="/settings">Settings</Link>
          <Can permission="whatsapp.manage_setup"><Link to="/settings/whatsapp-setup">WhatsApp Setup</Link></Can>
