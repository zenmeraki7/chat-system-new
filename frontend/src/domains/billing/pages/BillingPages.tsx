...
function RevenuePage() {
  ...
  const load = async (cursorValue?: string) => {
    let active = true;
    await Promise.all([
      api<any>("/billing/usage-summary?period=current_month"),
      api<unknown>(`/billing/ledger?${toQueryString({ cursor: cursorValue || undefined, limit: 50 })}`),
      api<any>("/analytics/campaign-performance?period=current_month"),
    ])
      .then(([usage, ledgerPayload, performance]) => {
        if (!active) return;
        const normalizedLedger = normalizeQueryPage<any>(ledgerPayload);
        setUsageSummary(usage || null);
        setLedger(normalizedLedger.items);
        setCampaignPerformance(performance || null);
        setCursor(cursorValue || "");
        setNextCursor(normalizedLedger.nextCursor || "");
      })
      .catch((e) => {
        if (!active) return;
  ...
  return (
    <Layout>
      ...
      <section className="kpi-grid">
        <article className="kpi-card"><h4>Month spend</h4><p>{usageSummary?.currency || "USD"} {Number(usageSummary?.charges_total || 0).toFixed(2)}</p></article>
        <article className="kpi-card"><h4>Refunds</h4><p>{usageSummary?.currency || "USD"} {Number(usageSummary?.refunds_total || 0).toFixed(2)}</p></article>
        <article className="kpi-card"><h4>Credits</h4><p>{usageSummary?.currency || "USD"} {Number(usageSummary?.credits_total || 0).toFixed(2)}</p></article>
        <article className="kpi-card"><h4>Net billed</h4><p>{usageSummary?.currency || "USD"} {Number(usageSummary?.net_total || 0).toFixed(2)}</p></article>
        <article className="kpi-card"><h4>Delivered</h4><p>{Number(campaignPerformance?.delivered_count || 0)}</p></article>
        <article className="kpi-card"><h4>Failed</h4><p>{Number(campaignPerformance?.failed_count || 0)}</p></article>
      </section>
      <section className="panel">
        <h3>Billing ledger</h3>
        <div className="campaign-card">
          {(ledger || []).length === 0 ? <p>No ledger entries found for this period.</p> : (
            <table>
              <thead>
                <tr>
                  <th>Date</th>
              ...
              <tbody>
                {(ledger || []).map((entry, idx) => (
                  <tr key={entry?.id || entry?.ledger_id || `ledger-${idx}`}>
                    <td>{entry?.created_at ? new Date(entry.created_at).toLocaleString() : "-"}</td>
                    <td>{String(entry?.entry_type || entry?.type || "-")}</td>
                    <td>{String(entry?.description || entry?.reason || "-")}</td>
                    <td>{`${entry?.currency || usageSummary?.currency || "USD"} ${Number(entry?.amount || 0).toFixed(2)}`}</td>
        ...
        <div className="campaign-actions">
          <button disabled={!nextCursor} onClick={() => load(nextCursor).catch((e) => safeLogClientError("ui_async_failed", e, { exposeToConsole: import.meta.env.DEV }))}>Next Ledger Page</button>
        </div>
      </section>
    </Layout>
  );
...
function WhatsAppSetupPage() {
  ...
  const readyToSend = String(status?.onboarding_state || "").toUpperCase() === "READY_TO_SEND";
  const steps = [
    { label: "Meta account", done: !!status?.token_valid && !tokenExpired, detail: status?.token_valid ? "OAuth token valid" : "Reconnect Meta OAuth" },
    { label: "WABA", done: hasWaba, detail: hasWaba ? status?.waba_id || "Selected" : "Choose or create WABA" },
    { label: "Phone", done: hasPhone, detail: hasPhone ? `${status?.display_phone_number || status?.phone_number_id || "Connected"} (${status?.display_name_status || "status unknown"})` : "Attach a sender number" },
    { label: "Cloud API registration", done: Boolean(status?.cloud_api_registered), detail: status?.cloud_api_registered ? "Registered" : "Registration required" },
    { label: "Operational test", done: Boolean(status?.test_message_passed), detail: status?.test_message_passed ? "Probe passed" : "Run test message probe" },
